...
int main(int argc, char ** argv)
{
	...
	int elmtSize = 7;// it is the size for our morphological element, can be changed depending on how much our object to be affected

	// Paths for the dataset
	// In this example we assume that the dataset is available at
	// "/home/avsa/datasets/dataset2012lite/dataset/baseline/...
		string dataset_path = "/home/backrest/eclipse-workspace-OpenCV/dynamicBackground/dynamicBackground"; //SET THIS DIRECTORY according to your download
...
//	string dataset_path = "/home/backrest/eclipse-workspace-OpenCV/Dataset/AVSASlidesVideos_dataset"; //SET THIS DIRECTORY according to your download
...
// if (argc==3)	{input=argv[3]);}
	...
	string project_root_path = "./AVSA2020results"; //SET THIS DIRECTORY according to your project
	string project_name = "Lab4AVSA2021"; // project exe name
	...
	for (int c=0; c<NumCat; c++ )
	{
		...
		string makedir_cmd = "mkdir -p "+results_path + "/" + dataset_cat[c];
		system(makedir_cmd.c_str()); //may raise error in console if path exists, but will work ...
...
//			namedWindow("ForeGround Mask"); // final fgmask
			...
			double tau = 40; // to set ... //Lab1.1.1-3
			double alpha=0.6; // to set ... //Lab1.1.2-3
			bool selective_bkg_update = false; // true ... //Lab1.1.2-3
			int threshold_ghosts2=0; // to set ... //Lab1.1.3
			bool rgb = false; //true;
			// bool rgb = true; //true;
			fgseg::bgs avsa_bgs(tau, rgb ); //construct object of the bgs class			//Lab1.1.2 fgseg::bgs avsa_bgs(tau, alpha, selective_bkg_update, rgb); //construct object of the bgs class
			...
			for (;;) {
				...
				avsa_bgs.Morpholody_Open(elmtSize);
				avsa_bgs.removeShadows();
				avsa_bgs.avsa_single_gaussian(alpha);
...
//		        imshow("ForeGround Mask", fgmask);
				...
				string inputFile=results_path + "/" + dataset_cat[c] + "/" + baseline_seq[s] + "/" + color_flag + tau_ss.str() + selective_flag + alpha_ss.str() + th_gh2_ss.str() + "input"+ it_ss.str() +".png";
